 ...
 */
    public function saleItem($cid,$date){
        $sql="SELECT dailymenu.cid, menuitem.mitemid, menuitem.fid, menuitem.isrecomd,food.fname, food.fdes, food.fprice, food.fpicture FROM(dailymenu JOIN menuitem ON dailymenu.mid=menuitem.mid)JOIN food ON food.fid=menuitem.fid WHERE dailymenu.cid='".$cid."'AND dailymenu.mdate='".$date."' AND menuitem.isrecomd='0' AND dailymenu.mstatus='1'";
        $query=$this->db->query($sql);
        if($query->num_rows()==2){
            return $query->result();
        }return false;
