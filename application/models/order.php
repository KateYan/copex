...
// generating vip user's order by using uid
    public function vipOrderByCard($uid, $vipid, $cid, $placeID, $odate, $fordate, $foodItem, $sideDishItem, $totalCost_beforTax)
    {
        ...
        //insert new order
        $sql = "INSERT INTO `order`(uid,cid,placeID,odate,fordate,oispaid,tax,totalcost) VALUES (" . $this->db->escape($uid) . "," . $this->db->escape($cid) . "," . $this->db->escape($placeID) . "," . $this->db->escape($odate) . "," . $this->db->escape($fordate) . "," . $this->db->escape($oispaid) . "," . $this->db->escape($tax) . "," . $this->db->escape($totalCost) . ") ";
        $this->db->query($sql);
...
//        $sql = "SELECT `order`.* ,`campus`.cname ,campus.caddr,orderitem.amount,orderitem.price,food.*,sidedish.* FROM ((`order` JOIN `campus` ON `order`.cid = `campus`.cid) JOIN orderitem ON `order`.oid = orderitem.oid) JOIN food ON orderitem.dishid=food.fid WHERE `order`.uid='$uid' AND `order`.fordate IN ('$today','$tomorrow')";
//
//        $query = $this->db->query($sql);
//        if ($query->num_rows() == 0) {
//            return false;
...
//        return $query->result();
    ...
    {
        // find order first
        $sql = "SELECT `order`.* ,`campus`.cname , campus.caddr, pickupplace.placeName,pickupplace.placeAddr FROM (`order` JOIN `campus` ON `order`.cid = `campus`.cid) LEFT JOIN pickupplace ON `order`.placeID = pickupplace.placeID WHERE `order`.uid='$uid' AND `order`.fordate IN ('$today','$tomorrow')";
        $query = $this->db->query($sql);
        if ($query->num_rows() == 0) {
            return false;
        }
    ...
    {
        $sql = "SELECT `order`.oid as orderNumber,`order`.oispaid as isPaid,`order`.ostatus as isPickedup,`campus`.cname as campus,`order`.fordate as forDate,`order`.odate as orderDate,`order`.totalcost as totalCost,`user`.uphone as userPhone,`user`.vipid as vipId,`user`.uid as userId, vipcard.vnumber as cardNumber FROM ((`order` JOIN campus ON `order`.cid = `campus`.cid) JOIN `user` ON  `order`.uid = `user`.uid) LEFT JOIN vipcard ON `user`.vipid = vipcard.vipid  WHERE `order`.cid='$campusId' AND `order`.fordate NOT IN('$date')ORDER BY `order`.oid DESC";
